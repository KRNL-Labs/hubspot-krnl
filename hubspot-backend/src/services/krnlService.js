...
class KRNLService {
  constructor() {
    ...
    this.mockMode = false; // No mocking for HubSpot integration
  ...
  async startComplianceWorkflow(params) {
    ...
    try {
      ...
      const safeWorkflowParams = workflowParams;
  ...
  async startAccessWorkflow(params) {
    ...
    try {
      const workflowTemplate = require('../../document-access-logging-workflow.json');
  ...
  async _waitForDocumentAccessLogged(txHash, documentHash, userId = null, accessType = null, startBlock = null) {
    ...
    const DOCUMENT_ACCESS_ABI = [
      'event DocumentAccessLogged(string documentHash, address accessor, string userId, string accessType, string documentId, bytes32 accessHash, uint256 timestamp)'
    ];
    ...
    const iface = new ethers.Interface(DOCUMENT_ACCESS_ABI);
    const eventTopic = iface.getEvent('DocumentAccessLogged').topicHash;
      ...
      for (const log of logs) {
        try {
          ...
          if (parsed && parsed.name === 'DocumentAccessLogged') {
            ...
            const loggedUserId = parsed.args.userId;
