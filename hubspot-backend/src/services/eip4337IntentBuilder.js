...
function getFunctionSelector(functionSignature) {
  const hash = ethers.keccak256(Buffer.from(functionSignature));
...
async function buildTransactionIntent4337(options = {}) {
  ...
  const value = 0n; // BigInt literal for ethers v6
  ...
  const signature = await wallet.signMessage(ethers.getBytes(intentHash));
