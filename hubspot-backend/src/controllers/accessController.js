...
const { saveSession, loadSession, loadAccessEvent, listAccessEvents } = require('../services/sessionStore');
...
router.post('/', async (req, res) => {
  try {
    ...
    // Direct access without authentication for HubSpot integration
...
router.get('/history/record/:recordId', async (req, res) => {
  try {
    ...
    logger.info('Access history by record requested', {
      recordId,
      limit,
      offset
    });
    ...
    const accessHistory = (result.records || []).map((row) => ({
      id: row.session_id,
    ...
    logger.error('Access history by record retrieval error', {
      error: error.message
    });
    res.status(500).json({
      success: false,
      error: 'Failed to retrieve access history by record',
      details: error.message
    });
  }
});
 ...
 */
router.get('/history/:documentHash', async (req, res) => {
  try {
 ...
 * Reads from krnl_access_events table (lean access records).
